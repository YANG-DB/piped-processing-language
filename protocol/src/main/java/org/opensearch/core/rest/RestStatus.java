...
public enum RestStatus {
  /**
   * The client SHOULD continue with its request. This interim response is used to inform the client
   ...
   * message header field (section 14.42), for a change in the application protocol being used on
   ...
   *
   * <ul>
   *   <li>GET: an entity corresponding to the requested resource is sent in the response;
   *   <li>HEAD: the entity-header fields corresponding to the requested resource are sent in the
   *       response without any message-body;
   *   <li>POST: an entity describing or containing the result of the action;
   *   <li>TRACE: an entity containing the request message as received by the end server.
   * </ul>
   */
  OK(200),
  /**
   ...
   * entity containing a list of resource characteristics and location(s) from which the user or
   * user agent can choose the one most appropriate. The entity format is specified by the media
   * type given in the Content-Type header field. The origin server MUST create the resource before
   ...
   *
   * <p>A 201 response MAY contain an ETag response header field indicating the current value of the
   * entity tag for the requested variant just created, see section 14.19.
   */
  CREATED(201),
  /**
   ...
   * a request for some other process (perhaps a batch-oriented process that is only run once per
   * day) without requiring that the user agent's connection to the server persist until the process
   * is completed. The entity returned with this response SHOULD include an indication of the
   * request's current status and either a pointer to a status monitor or some estimate of when the
   * user can expect the request to be fulfilled.
   */
  ACCEPTED(202),
  /**
   * The returned meta information in the entity-header is not the definitive set as available from
   * the origin server, but is gathered from a local or a third-party copy. The set presented MAY be
   * a subset or super set of the original version. For example, including local annotation
   * information about the resource might result in a super set of the meta information known by the
   ...
   * to return updated meta information. The response MAY include new or updated meta information in
   * the form of entity-headers, which if present SHOULD be associated with the requested variant.
   *
   * <p>If the client is a user agent, it SHOULD NOT change its document view from that which caused
   ...
   * updated meta information SHOULD be applied to the document currently in the user agent's active
   * view.
   *
   * <p>The 204 response MUST NOT include a message-body, and thus is always terminated by the first
   ...
   *
   * <ul>
   *   <li>Either a Content-Range header field (section 14.16) indicating the range included with
   *       this response, or a multipart/byteranges Content-Type including Content-Range fields for
   *       each part. If a Content-Length header field is present in the response, its value MUST
   *       match the actual number of OCTETs transmitted in the message-body.
   *   <li>Date
   *   <li>ETag and/or Content-Location, if the header would have been sent in a 200 response to the
   *       same request
   *   <li>Expires, Cache-Control, and/or Vary, if the field-value might differ from that sent in
   *       any previous response for the same variant
   * </ul>
   *
   * <p>If the 206 response is the result of an If-Range request that used a strong cache validator
   * (see section 13.3.3), the response SHOULD NOT include other entity-headers. If the response is
   * the result of an If-Range request that used a weak validator, the response MUST NOT include
   * other entity-headers; this prevents inconsistencies between cached entity-bodies and updated
   ...
   *
   * <p>A cache MUST NOT combine a 206 response with other previously cached content if the ETag or
   * Last-Modified headers do not match exactly, see 13.5.4.
   *
   * <p>A cache that does not support the Range and Content-Range headers MUST NOT cache 206
   * (Partial) responses.
   */
  PARTIAL_CONTENT(206),
  /**
   * The 207 (Multi-Status) status code provides status for multiple independent operations (see
   * Section 13 for more information).
   *
   * <p>A Multi-Status response conveys information about multiple resources in situations where
   * multiple status codes might be appropriate. The default Multi-Status response body is a
   * text/xml or application/xml HTTP entity with a 'multistatus' root element. Further elements
   * contain 200, 300, 400, and 500 series status codes generated during the method invocation. 100
   * series status codes SHOULD NOT be recorded in a 'response' XML element.
   *
   * <p>Although '207' is used as the overall response status code, the recipient needs to consult
   * the contents of the multistatus response body for further information about the success or
   * failure of the method execution. The response MAY be used in success, partial success and also
   * in failure situations.
   *
   * <p>The 'multistatus' root element holds zero or more 'response' elements in any order, each
   * with information about an individual resource. Each 'response' element MUST have an 'href'
   ...
   * that location.
   *
   * <p>Unless it was a HEAD request, the response SHOULD include an entity containing a list of
   * resource characteristics and location(s) from which the user or user agent can choose the one
   * most appropriate. The entity format is specified by the media type given in the Content-Type
   * header field. Depending upon the format and the capabilities of the user agent, selection of
   * the most appropriate choice MAY be performed automatically. However, this specification does
   * not define any standard for such automatic selection.
   *
   * <p>If the server has a preferred choice of representation, it SHOULD include the specific URI
   * for that representation in the Location field; user agents MAY use the Location field value for
   * automatic redirection. This response is cacheable unless indicated otherwise.
   */
   ...
   * automatically re-link references to the Request-URI to one or more of the new references
   * returned by the server, where possible. This response is cacheable unless indicated otherwise.
   *
   * <p>The new permanent URI SHOULD be given by the Location field in the response. Unless the
   * request method was HEAD, the entity of the response SHOULD contain a short hypertext note with
   * a hyperlink to the new URI(s).
   *
   * <p>If the 301 status code is received in response to a request other than GET or HEAD, the user
   * agent MUST NOT automatically redirect the request unless it can be confirmed by the user, since
   * this might change the conditions under which the request was issued.
   */
  MOVED_PERMANENTLY(301),
  /**
   * The requested resource resides temporarily under a different URI. Since the redirection might
   ...
   *
   * <p>The temporary URI SHOULD be given by the Location field in the response. Unless the request
   * method was HEAD, the entity of the response SHOULD contain a short hypertext note with a
   * hyperlink to the new URI(s).
   *
   * <p>If the 302 status code is received in response to a request other than GET or HEAD, the user
   * agent MUST NOT automatically redirect the request unless it can be confirmed by the user, since
   * this might change the conditions under which the request was issued.
   */
  FOUND(302),
   ...
   * substitute reference for the originally requested resource. The 303 response MUST NOT be
   * cached, but the response to the second (redirected) request might be cacheable.
   *
   * <p>The different URI SHOULD be given by the Location field in the response. Unless the request
   * method was HEAD, the entity of the response SHOULD contain a short hypertext note with a
   * hyperlink to the new URI(s).
   */
  SEE_OTHER(303),
   ...
   *
   * <ul>
   *   <li>Date, unless its omission is required by section 14.18.1 If a clockless origin server
   *       obeys these rules, and proxies and clients add their own Date to any response received
   *       without one (as already specified by [RFC 2068], section 14.19), caches will operate
   *       correctly.
   *   <li>ETag and/or Content-Location, if the header would have been sent in a 200 response to the
   *       same request
   *   <li>Expires, Cache-Control, and/or Vary, if the field-value might differ from that sent in
   ...
   * </ul>
   *
   * <p>If the conditional GET used a strong cache validator (see section 13.3.3), the response
   * SHOULD NOT include other entity-headers. Otherwise (i.e., the conditional GET used a weak
   * validator), the response MUST NOT include other entity-headers; this prevents inconsistencies
   * between cached entity-bodies and updated headers.
   *
   * <p>If a 304 response indicates an entity not currently cached, then the cache MUST disregard
   * the response and repeat the request without the conditional.
   *
   * <p>If a cache uses a received 304 response to update a cache entry, the cache MUST update the
   ...
   * Location field gives the URI of the proxy. The recipient is expected to repeat this single
   * request via the proxy. 305 responses MUST only be generated by origin servers.
   */
  USE_PROXY(305),
  /**
   ...
   * status. Therefore, the note SHOULD contain the information necessary for a user to repeat the
   * original request on the new URI.
   *
   * <p>If the 307 status code is received in response to a request other than GET or HEAD, the user
   * agent MUST NOT automatically redirect the request unless it can be confirmed by the user, since
   * this might change the conditions under which the request was issued.
   */
   ...
   *
   * <p>Note: HTTP/1.1 servers are allowed to return responses which are not acceptable according to
   * the accept headers sent in the request. In some cases, this may even be preferable to sending a
   * 406 response. User agents are encouraged to inspect the headers of an incoming response to
   * determine if it is acceptable.
   *
   * <p>If the response could be unacceptable, a user agent SHOULD temporarily stop receipt of more
   ...
   *
   * <p>Conflicts are most likely to occur in response to a PUT request. For example, if versioning
   ...
   *
   * <p>The 410 response is primarily intended to assist the task of web maintenance by notifying
   * the recipient that the resource is intentionally unavailable and that the server owners desire
   ...
   *
   * <p>If the condition is temporary, the server SHOULD include a Retry-After header field to
   ...
   *
   * <p>When this status code is returned for a byte-range request, the response SHOULD include a
